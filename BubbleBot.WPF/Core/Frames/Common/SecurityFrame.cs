...
namespace BubbleBot.Core.Frames.Common
{
    ...
    {
        public static async Task HandleBasicLatencyStatsRequestMessage(Account account,
            BasicLatencyStatsRequestMessage message)
        {
            await account.Network.SendMessageAsync(new BasicLatencyStatsMessage(262, 12, 50)).ConfigureAwait(false);
        }

        public static async Task HandleSequenceNumberRequestMessage(Account account,
            SequenceNumberRequestMessage message)
        {
            account.FramesData.Sequence++;
            await account.Network.SendMessageAsync(new SequenceNumberMessage(account.FramesData.Sequence))
        ...
        public static Task HandleRecaptchaRequestMessage(Account account, RecaptchaRequestMessage message)
        {
            return Task.Factory.StartNew(async () =>
            {
                ...
                account.Logger.LogWarning(LanguageManager.Translate("71"),
                    LanguageManager.Translate("72", account.FramesData.CaptchasCounter));
                ...
                try
                {
                    ...
                    if (account.Game.Character != null && account.Game.Character.IsSelected)
                        foreach (var acc in BubbleBotMain.Instance.EveryConnectedAccount())
                            ...
                            {
                                acc.Logger.LogWarning(LanguageManager.Translate("654"),
                                    LanguageManager.Translate("655", account.Game.Character.Name,
                                        account.Game.Server.Name));
                            }
