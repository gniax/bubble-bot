...
namespace BubbleBot.Core.Accounts.Scripts
{
    public class ScriptsManager : ViewModelBase, IDisposable
    {
        ...
        public bool Running => _account?.IsGroupChief == true
            ? Enabled && !Paused
            : _account?.Group.Chief.Scripts.Running == true;

        public LuaScriptManager ScriptManager =>
            _account.IsGroupChief ? _scriptManager : _account.Group.Chief.Scripts._scriptManager;
        ...
        private void BeforeDoFile()
        {
            ...
            ScriptManager.SetGlobal("loadConfigurationFunc",
                new Action<string>(msg => ActionsManager.EnqueueAction(new LoadConfigurationAction(msg), true)));
            ScriptManager.SetGlobal("mapTeleportation",
                new Action(() => ActionsManager.EnqueueAction(new MapTeleportationAction(), true)));
            ScriptManager.SetGlobal("bypassTutorial",
                new Action(() => ActionsManager.EnqueueAction(new GuidedModeQuit(), true)));
            ScriptManager.SetGlobal("subscribeFunc",
                new Action(() => ActionsManager.EnqueueAction(new SubscribeAction(), true)));
            ScriptManager.SetGlobal("printMessage",
                new Action<string>(msg => _account.Logger.LogMessage(LanguageManager.Translate("165"), msg)));
            ScriptManager.SetGlobal("printDebug",
                new Action<string>(msg => _account.Logger.LogDebug(LanguageManager.Translate("165"), msg)));
            ScriptManager.SetGlobal("printSuccess",
                new Action<string>(msg => _account.Logger.LogInfo(LanguageManager.Translate("165"), msg)));
            ScriptManager.SetGlobal("printError",
                new Action<string>(msg => _account.Logger.LogError(LanguageManager.Translate("165"), msg)));
            ...
            ScriptManager.SetGlobal("getSubscriptionPrice",
                new Func<double>(() => { return Math.Ceiling(_account.Game.bakRate * 800); }));
            ScriptManager.SetGlobal("reconnectFunc",
                new Action<int, bool>((s, restartscript) =>
                    ActionsManager.EnqueueAction(new ReconnectAction(s, restartscript), true)));
                ...
                for (var i = 0; i < 24; i++)
                    if (hour < i && !_account.AccountConfig.Planification[i])
                        return DateTimeOffset.UtcNow.ToUnixTimeSeconds() +
                               (60 * 60 * (i - hour) -
                                (DateTimeOffset.UtcNow.Minute * 60 + DateTimeOffset.UtcNow.Second));
                return 0;
            }));

            // Inject api script
            ScriptManager.Script.DoString(Resources.scriptsApiHelper);
        }
        ...
        public void StopScript(string reason = "")
        {
            ...
            if (_account.HasGroup && _account.IsGroupChief) _account.Group.MembersCleaningAndClearing();
        ...
        private void ParseEntry(Table entry)
        {
            ...
            if (CurrentFunctionType == FunctionTypes.MOVE)
            {
                ...
                if (!flag.IsNil() && flag.Type == DataType.Boolean && flag.Boolean) _entryFlags.Add(new GatherFlag());

                // fight
                flag = entry.Get("fight");
                if (!flag.IsNil() && flag.Type == DataType.Boolean && flag.Boolean) _entryFlags.Add(new FightFlag());
            }
            ...
            if (CurrentFunctionType == FunctionTypes.BANK)
            {
                ...
                if (!flag.IsNil() && flag.Type == DataType.Boolean && flag.Boolean) _entryFlags.Add(new NpcBankFlag());
            }
            ...
            if (CurrentFunctionType == FunctionTypes.PHENIX)
            {
                ...
                if (!flag.IsNil() && flag.Type == DataType.String && short.TryParse(flag.String, out var phenixCellId))
                    _entryFlags.Add(new PhenixFlag(phenixCellId));
            }

            // custom
            flag = entry.Get("custom");
            if (!flag.IsNil() && flag.Type == DataType.Function) _entryFlags.Add(new CustomFlag(flag));

            // door
            flag = entry.Get("door");
            if (!flag.IsNil() && flag.Type == DataType.String && short.TryParse(flag.String, out var doorCellId))
                _entryFlags.Add(new DoorFlag(doorCellId));

            // changeMap
            flag = entry.Get("changeMap");
            if (!flag.IsNil() && flag.Type == DataType.String) _entryFlags.Add(new ChangeMapFlag(flag.String));

            // Check if there is at least one valid flag
            if (_entryFlags.Count == 0) StopScript(LanguageManager.Translate("144") + " 1");
        }
            ...
            if (!avoidChecks)
                ...
                switch (_entryFlags[_entryFlagsIndex])
                {
                    ...
                    case FightFlag _:
                        ...
                        if (_account.Game.Map.CanFight(fightAction.MinMonsters, fightAction.MaxMonsters,
        ...
        {
            var maxPods = ScriptManager.GetGlobalOr(LanguageManager.Translate("145"), DataType.Number, 90);
        ...
        private async Task CheckScriptRegeneration()
        {
            ...
            var minLife = autoRegen.GetOr("minLife", DataType.Number, 0);
            var maxLife = autoRegen.GetOr("maxLife", DataType.Number, 100);
            ...
            var endHp = maxLife * (int) _account.Game.Character.Stats.MaxLifePoints / 100;
            var hpToRegen = endHp - (int) _account.Game.Character.Stats.LifePoints;
            ...
            for (var i = 0; i < items.Count; i++)
            {
                ...
                var neededQty = (int) Math.Floor(hpToRegen / (double) obj.RegenValue);
                // Then get how much qty we actually have
                var validQty = Math.Min(neededQty, (int) obj.Quantity);
        ...
        private async Task CheckRegeneration()
        {
            ...
            if (_account.Extensions.Fights.Configuration.RegenEnd <=
                _account.Extensions.Fights.Configuration.RegenStart)
                return;
            ...
            if (_account.Game.Character.Stats.LifePercent <= _account.Extensions.Fights.Configuration.RegenStart)
            {
                ...
                var endHp = _account.Extensions.Fights.Configuration.RegenEnd *
                    (int) _account.Game.Character.Stats.MaxLifePoints / 100;
                var hpToRegen = endHp - (int) _account.Game.Character.Stats.LifePoints;

                if (hpToRegen > 0)
                {
                    var estimatedTime = hpToRegen / 2;

                    // Use sit emote for double hp regen (if we're not already sitting)
                    if (_account.State != AccountStates.REGENERATING) _account.Game.Character.Sit();

                    _account.Logger.LogInfo(LanguageManager.Translate("165"),
                        LanguageManager.Translate("150", hpToRegen, estimatedTime));

                    // Then just wait it before continuing
                    for (var i = 0;
                        i < estimatedTime && _account.Game.Character.Stats.LifePercent <=
                        _account.Extensions.Fights.Configuration.RegenEnd && Running;
        ...
        private async Task CheckForBags()
        {
            ...
            var bags = _account.Game.Character.Inventory.Objects.Where(o => o.TypeId == 100 && o.SuperTypeId == 6)
                .ToList();
        ...
        {
            // Stat
            if (_account.Configuration.StatToBoost != BoostableStats.NONE &&
                _account.Game.Character.Stats.StatsPoints > 0)
        ...
        private async Task CheckForAutoDelete()
        {
            ...
            var autoDeleteElements =
                ScriptManager.GetGlobalOr<Table>(LanguageManager.Translate("147"), DataType.Table, null);
        ...
        {
            if (!_account.Configuration.AutoMount || !_account.Game.Character.Mount.HasMount ||
        ...
        private bool CheckForSpecialCases([CallerMemberName] string caller = "")
        {
            ...
            if (CurrentFunctionType == FunctionTypes.PHENIX &&
                (_account.HasGroup && _account.IsGroupChief
                    ? _account.Group.IsEveryoneAliveAndKicking()
            ...
            if (CurrentFunctionType == FunctionTypes.BANK && (_account.HasGroup && _account.IsGroupChief
                ? !_account.Group.IsAnyoneFullWeight()
                : !GotToMaxPods()))
            {
        ...
        private GatherAction CreateGatherAction()
        {
            ...
            var ressourcesIds = new List<int>();
            ...
            if (ressourcesIds.Count == 0) ressourcesIds.AddRange(_account.Game.Character.Jobs.GetCollectSkillsIds());
        ...
        private void HandleFightFlag(FightAction fightAction)
        {
            ...
            var maxFightsPerMap = ScriptManager.GetGlobalOr("MAX_FIGHTS_PER_MAP", DataType.Number, -1);
            ...
            if (_account.Game.Map.CanFight(action.MinMonsters, action.MaxMonsters, action.MinMonstersLevel,
        ...
        {
            var minMonsters = ScriptManager.GetGlobalOr("MIN_MONSTERS", DataType.Number, 1);
            var maxMonsters = ScriptManager.GetGlobalOr("MAX_MONSTERS", DataType.Number, 8);
            var minLevel = ScriptManager.GetGlobalOr("MIN_MONSTERS_LEVEL", DataType.Number, 1);
            var maxLevel = ScriptManager.GetGlobalOr("MAX_MONSTERS_LEVEL", DataType.Number, 1000);
        ...
        private void HandleNpcBankFlag()
        {
            ...
            if (bankPutItems != null && bankPutItems.Length > 0)
                foreach (var val in bankPutItems.Values)
                {
                    ...
                    if (item.IsNil() || item.Type != DataType.Number || quantity.IsNil() ||
                        quantity.Type != DataType.Number)
                        continue;
            ...
            if (autoRegen != null)
            {
                var store = autoRegen.GetOr("store", DataType.Number, 0);
                if (store > 0)
                    ActionsManager.EnqueueAction(
                        new StorageGetAutoRegenStoreAction(autoRegen.Get("items").ToObject<List<int>>(), store));
            }

            // BANK_PUT_KAMAS
            var amount = ScriptManager.GetGlobalOr("BANK_PUT_KAMAS", DataType.Number, -1);
            if (amount >= 0) ActionsManager.EnqueueAction(new StoragePutKamasAction(amount));

            // BANK_GET_KAMAS
            amount = ScriptManager.GetGlobalOr("BANK_GET_KAMAS", DataType.Number, -1);
            if (amount >= 0) ActionsManager.EnqueueAction(new StorageGetKamasAction(amount));
        ...
        {
            if (ChangeMapAction.TryParse(cmf.Where, out var action))
                ActionsManager.EnqueueAction(action, true);
            else
