...
{
    class SubscribeAction : ScriptAction
    {
        // Constructor
        ...
        internal async override Task<ScriptActionResults> Process(Account account)
        {
            ...
            double subprice = (Math.Ceiling(account.Game.bakRate * 800));
            ...
            await Task.Delay(1500);
            account.Logger.LogInfo(LanguageManager.Translate("606"), LanguageManager.Translate("623", subprice.ToString()));
            await account.Network.SendCallAsync(new shopBuyRequest("KMS", 800, Convert.ToInt64(subprice), 1, 7537, false)).ConfigureAwait(false);
            ...
            switch (account.Game.shopBuyInfo)
            {
                ...
                case 1:
                    ...
                    DateTime newDate = localDate.AddDays(7);
