...
namespace BubbleBot.Core.Commands.Handlers
{
    public class HelpCommands
    {
        ...
        public static Task MapIdCommand(Account account)
        {
            return Task.Run(() =>
            {
                account.Logger.LogInfo(LanguageManager.Translate("199"),
                    LanguageManager.Translate("200", account.Game.Map.Id));
            });
        }
        ...
        public static Task CellIdCommand(Account account)
        {
            ...
            {
                account.Logger.LogInfo(LanguageManager.Translate("199"),
                    LanguageManager.Translate("202", account.Game.Map.PlayedCharacter.CellId));
            });
        }
            ...
            return Task.Run(() =>
            {
                ...
                else
                {
                    ...
                    foreach (var p in account.Game.Map.Players)
                        account.Logger.LogInfo(LanguageManager.Translate("199"),
                            $"   [{p.Id}] {p.Name} sur la cellule: {p.CellId}");
                }
                ...
                else
                {
                    ...
                    foreach (var n in account.Game.Map.Npcs)
                        account.Logger.LogInfo(LanguageManager.Translate("199"),
                            $"   [{n.Id}] ({n.NpcId}) {n.Name} sur la cellule: {n.CellId}");
                }
            });
        }
        ...
        public static Task InteractivesCommand(Account account)
        {
            return Task.Run(() =>
            {
                ...
                {
                    account.Logger.LogInfo(LanguageManager.Translate("199"),
                        "Il n'y pas d'éléments interactifs sur cette map.");
                    return;
                }
        ...
        {
            return Task.Run(() => account.Network.SendMessage(new FriendJoinRequestMessage(player)));
        }
        ...
        public static Task UseZaapCommand(Account account, string destinationMapId)
        {
            return Task.Run(() =>
            {
                if (uint.TryParse(destinationMapId, out var mapId))
